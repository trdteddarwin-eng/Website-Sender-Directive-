...
For fully automated runs, use the daily pipeline instead of manual steps:

```bash
python execution/upwork_daily_pipeline.py              # Full automated run
python execution/upwork_daily_pipeline.py --dry-run     # Test without Opus costs
```

This uses **Bright Data Web Unlocker** (replacing Apify), adds AI relevance scoring via Sonnet before generating proposals, and handles deduplication automatically. See `directives/upwork_daily_pipeline.md` for full docs.

**Key scripts:**
- `execution/upwork_daily_pipeline.py` — Main orchestrator
- `execution/upwork_brightdata_scraper.py` — Bright Data scraper (replaces `upwork_apify_scraper.py`)
- `execution/upwork_pipeline_config.json` — Config (keywords, filters, scoring)

## Learnings

- Apify free tier only filters: `limit`, `fromDate`, `toDate` - all other filters are post-scrape
- **Bright Data Web Unlocker** is the preferred scraper (bypasses Cloudflare, parses `__NEXT_DATA__`)
...
- Sonnet 4.5 model ID: `claude-sonnet-4-5-20250929` (used for cheap relevance scoring)
...
- 3-phase qualification (deterministic → Sonnet → Opus) saves ~50% on Opus costs
