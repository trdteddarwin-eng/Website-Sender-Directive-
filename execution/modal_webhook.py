...
ALL_TOOLS = {
    ...
    "google_reviews_api": {
        "name": "google_reviews_api",
        "description": "Fetch and reply to Google reviews for a business location via Google My Business API.",
        "input_schema": {
            "type": "object",
            "properties": {
                "action": {"type": "string", "enum": ["fetch", "reply"], "description": "Whether to fetch reviews or reply to one"},
                "location_id": {"type": "string", "description": "Google My Business location ID"},
                "review_id": {"type": "string", "description": "Review ID (required for reply action)"},
                "reply_text": {"type": "string", "description": "Reply text (required for reply action)"},
                "page_size": {"type": "integer", "description": "Number of reviews to fetch (default 10)", "default": 10}
            },
...
def google_reviews_api_impl(action: str, location_id: str, token_data: dict,
                             review_id: str = None, reply_text: str = None,
                             page_size: int = 10) -> dict:
    """Fetch or reply to Google reviews via Google My Business API."""
    from google.oauth2.credentials import Credentials
    from googleapiclient.discovery import build
    from google.auth.transport.requests import Request
    ...
    if not account_id:
        return {"error": "GMB_ACCOUNT_ID environment variable not set"}
        ...
        try:
            ...
            for review in all_reviews:
                ...
                formatted.append({
                    ...
                    "rating": rating_map.get(review.get("starRating", "STAR_RATING_UNSPECIFIED"), 0),
            ...
            logger.info(f"Fetched {len(formatted)} reviews from {location_id}")
            return {"reviews": formatted, "total": len(formatted), "location": location_id}

        except Exception as e:
            logger.error(f"Error fetching reviews for {location_id}: {e}")
            return {"error": f"Failed to fetch reviews: {str(e)}", "location": location_id}

    elif action == "reply":
        if not review_id:
            return {"error": "review_id is required for reply action"}
        if not reply_text:
            return {"error": "reply_text is required for reply action"}

        try:
            # Build the review name path
            review_name = f"{parent}/reviews/{review_id}"
            ...
            logger.info(f"Replied to review {review_id} at {location_id}")
            return {
                "status": "replied",
                "review_id": review_id,
                "location": location_id,
            ...
            logger.error(f"Error replying to review {review_id}: {e}")
            return {"error": f"Failed to reply to review: {str(e)}", "review_id": review_id}

    else:
        return {"error": f"Unknown action: {action}. Use 'fetch' or 'reply'."}
...
TOOL_IMPLEMENTATIONS = {
    ...
    "google_reviews_api": lambda **kwargs: google_reviews_api_impl(**kwargs),
}

# Tools that need token_data
TOOLS_NEEDING_TOKEN = {"send_email", "read_sheet", "update_sheet", "google_reviews_api"}
...
@app.function(image=image, secrets=ALL_SECRETS, timeout=10)
@modal.fastapi_endpoint(method="GET")
def pixel(slug: str):
    """
    Tracking pixel endpoint. Embed in outreach emails as:
        <img src="https://nick-90891--claude-orchestrator-pixel.modal.run?slug={slug}" width="1" height="1" />

    Logs email open to Slack and returns a 1x1 transparent PNG.
    """
    from fastapi.responses import Response

    timestamp = datetime.utcnow().isoformat()
    logger.info(f"EMAIL OPEN: {slug} at {timestamp}")

    # Log to Slack
    try:
        slack_url = os.getenv("SLACK_WEBHOOK_URL")
        if slack_url:
            message = f"📬 Email opened: *{slug}* at {timestamp}"
...
@app.function(image=image, secrets=ALL_SECRETS, timeout=10)
@modal.fastapi_endpoint(method="GET")
def click(slug: str, url: str = ""):
    """
    Click tracking endpoint. Wrap spec site links in emails as:
        <a href="https://nick-90891--claude-orchestrator-click.modal.run?slug={slug}&url={encoded_spec_url}">

    Logs click to Slack, then redirects to the spec site URL.
    """
    from fastapi.responses import RedirectResponse

    timestamp = datetime.utcnow().isoformat()
    logger.info(f"LINK CLICK: {slug} → {url} at {timestamp}")

    # Log to Slack
    try:
        slack_url = os.getenv("SLACK_WEBHOOK_URL")
        if slack_url:
            message = f"🔗 Link clicked: *{slug}* → {url} at {timestamp}"
            data = json.dumps({"text": message}).encode("utf-8")
            req = urllib.request.Request(
                slack_url,
                data=data,
    ...
    if url:
        ...
        return {"status": "clicked", "slug": slug, "timestamp": timestamp}
