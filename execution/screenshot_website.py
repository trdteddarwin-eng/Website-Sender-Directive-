...
def screenshot_website(url, output_path, viewport_width=1440, viewport_height=900, full_page=True, timeout=30000, compress=False):
    ...
    Returns:
        ...
        Also writes a .meta.json sidecar with {"usable": bool, "reason": str}.
    """
    # Ensure output directory exists
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)

    meta = {"usable": True, "reason": "ok"}
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            context = browser.new_context(
                ...
                device_scale_factor=1,  # 1x for email-sized screenshots (1440px is plenty)
            )
            page = context.new_page()

            print(f"Loading: {url}")
            # Use "load" instead of "networkidle" — networkidle hangs on sites
            # with persistent connections (chat widgets, analytics, etc.)
            try:
                page.goto(url, wait_until="load", timeout=timeout)
            except Exception:
                # Fallback: try domcontentloaded if load times out
                try:
            ...
            # Check if page is an error/maintenance/login page (unusable for emails)
            page_check = page.evaluate("""() => {
                const text = (document.body ? document.body.innerText : '').toLowerCase();
                const title = (document.title || '').toLowerCase();
                const all = text + ' ' + title;

                // Maintenance / coming soon / under construction
                const maintenancePatterns = [
                    'down for maintenance', 'under maintenance', 'under construction',
                    'coming soon', 'check back', 'be right back',
                    'temporarily unavailable', 'site is down',
                ];
                for (const p of maintenancePatterns) {
                    if (all.includes(p)) return {usable: false, reason: 'maintenance'};
                }

                // Login walls / password pages
                const loginPatterns = [
                    'password protected', 'enter password', 'access denied',
                ];
                const hasLoginForm = document.querySelector('input[type="password"]') !== null;
                const bodyLen = text.length;
                if (hasLoginForm && bodyLen < 500) {
                    for (const p of loginPatterns) {
                        if (all.includes(p)) return {usable: false, reason: 'login_wall'};
                    }
                    // Small page with a password field = likely a login gate
                    if (all.includes('log in') || all.includes('login') || all.includes('sign in')) {
                        return {usable: false, reason: 'login_wall'};
                    }
                }
                ...
                const errorPatterns = [
                    ...
                    'err_connection_refused', 'dns_probe_finished',
                ];
                for (const p of errorPatterns) {
                    if (all.includes(p)) return {usable: false, reason: 'error_page'};
                }

                // Parked / expired domains
                const parkedPatterns = [
                    'this domain is for sale', 'domain expired',
                    'buy this domain', 'parked domain', 'godaddy',
                ];
                for (const p of parkedPatterns) {
                    if (all.includes(p)) return {usable: false, reason: 'parked_domain'};
                }
        ...
        if not meta.get("usable", True):
            print(f"Screenshot saved but UNUSABLE ({meta['reason']}): {output_path}")
        else:
        ...
        meta = {"usable": False, "reason": f"capture_failed: {str(e)[:100]}"}
        result = generate_placeholder(output_path, url, viewport_width, viewport_height, str(e))
        _write_meta(result, meta)
        return result


def _write_meta(screenshot_path, meta):
    """Write a .meta.json sidecar next to the screenshot."""
    import json
    base = screenshot_path.rsplit(".", 1)[0]
    meta_path = base + ".meta.json"
    with open(meta_path, "w") as f:
        json.dump(meta, f)


def is_screenshot_usable(screenshot_path):
    """Check if a screenshot is usable (not a maintenance/error/login page).

    Reads the .meta.json sidecar written by screenshot_website().
    Returns True if usable or if no meta file exists (assume usable).
    """
    import json
    base = screenshot_path.rsplit(".", 1)[0]
    # Check both .png and .jpg meta
    for ext_base in [base, screenshot_path.replace(".jpg", "").replace(".png", "")]:
        meta_path = ext_base + ".meta.json"
        if os.path.exists(meta_path):
            try:
                with open(meta_path) as f:
                    meta = json.load(f)
                return meta.get("usable", True)
            except Exception:
                pass
    return True  # No meta = assume usable
